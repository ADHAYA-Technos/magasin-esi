...
const handleFournisseurChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  const handleAddRow = () => {
    ...
    if (
      ...
      if (lastProduct && lastProduct.productId) {
        const confirmed = window.confirm('Price and quantity fields in the current row are not filled. Do you want to proceed anyway?');
        if (confirmed) {
  ...
  const handleConfirm = async () => {
    try {
      ...
      if (!orderRecipient.chapitre || !orderRecipient.article || !orderRecipient.fournisseur || orderRecipient.products.length === 0) {
  ...
  return (
    <>
      ...
      {orderRecipient.products.map((product, index) => (
        ...
        <button onClick={() => handleDeleteRow(index)} className="bg-red-500 text-white py-2 px-4 rounded-md shadow-md hover:bg-red-600 focus:outline-none focus:ring focus:ring-red-300 mr-6">
          Delete
        </button>
